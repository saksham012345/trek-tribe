...
(async () => {
  ...
  try {
    ...
    const target = dbs.databases.map(d => d.name).filter(n => n === 'test' || n.startsWith('trek') || n === 'trekktribe');
    if (target.length === 0) {
      ...
      for (const c of cols) {
        try {
          ...
          if (cnt && cnt > 0) {
            ...
            // Mask obvious sensitive fields (like password hashes, tokens) before printing
            if (sample) {
              const masked = { ...sample };
