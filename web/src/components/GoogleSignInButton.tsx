...
const GoogleSignInButton: React.FC<GoogleSignInButtonProps> = ({
  ...
  const initializeGoogleSignIn = () => {
    ...
    // Note: In production, you'd get this from environment variables
  ...
  const handleGoogleSignIn = async (response: any) => {
    ...
    try {
      ...
      const result = await axios.post('/auth/google/token', {
  ...
  const handleManualSignIn = async () => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.error || 'Failed to initiate Google authentication';
      onError?.(errorMessage);
      setIsLoading(false);
    }
  };
  ...
  return (
    <button
      ...
      className={`
        ...
        bg-white text-gray-700 font-medium hover:bg-gray-50 hover:border-gray-400
