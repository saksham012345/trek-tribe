# Security Check Script for Trekk Tribe
...
if ($dbUrls) {
    Write-Host "ERROR: Found hardcoded database credentials in commit!" -ForegroundColor Red
    $hasErrors = $true
}
...
if ($jwtSecrets) {
    Write-Host "ERROR: Found hardcoded JWT secret in commit!" -ForegroundColor Red
    $hasErrors = $true
}
