    ...
    try {
        ...
        try {
            ...
            try {
                ...
                agentToken = signupRes.data.token;
        ...
        for (const traveler of TRAVELER_USERS) {
            try {
                ...
                travelerTokens[traveler.email] = loginRes.data.token;
                ...
                try {
                    ...
                    travelerTokens[traveler.email] = signupRes.data.token;
        ...
        for (let i = 0; i < Math.min(PREMIUM_TICKET_TEMPLATES.length, travelerEmails.length); i++) {
            ...
            try {
                const ticketRes = await axios_1.default.post(`${API_URL}/support/tickets`, {
        ...
        for (const ticket of createdTickets) {
            try {
                await axios_1.default.post(`${API_URL}/agent/tickets/${ticket.ticketId}/assign`, {}, // The agent will auto-assign to themselves
                {
        ...
        for (let i = 0; i < createdTickets.length; i++) {
            ...
            try {
                // Add agent message
                const messageRes = await axios_1.default.post(`${API_URL}/agent/tickets/${ticket.ticketId}/messages`, {
                ...
                // Resolve ticket
                const resolveRes = await axios_1.default.post(`${API_URL}/agent/tickets/${ticket.ticketId}/resolve`, {
